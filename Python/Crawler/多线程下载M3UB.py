...
class M3u8:
    ...
    def progressBar(self, queue, count):
        ...
        while True:
            ...
            print("\r%s下载成功，当前进度%0.2f%%, 第%s/%s个" %
                  (file, rate, offset, count))
            if offset >= count:
                break
    ...
    def run(self):
        ...
        for file in container:
            ...
            po.apply_async(self.download, args=(
                queue, sort, file, downPath, url,))
            size += 1
        ...
        print('---文件合并清除...')
        if sys == "Windows":
            os.system("copy /b {}/*.ts {}/{}.{}".format(downPath,
                                                        savePath, saveName, saveSuffix))
            if clearDebris:
                os.system("rmdir /s/q {}".format(downPath))
        else:
            os.system("cat {}/*.ts>{}/{}.{}".format(downPath,
                                                    savePath, saveName, saveSuffix))
            if clearDebris:
