...
Function login
        ...
        flag =1 
        While flag = 1 
                username = crt.Dialog.Prompt("请输入Local IP,使用默认可不填IP", "IP", "192.168.99.208", False)
        ...
        passwd = crt.Dialog.Prompt("请输入密码,使用默认密码可不填", "Root密码", "WAP_hsl3fc01",true)

        '连接主机
        crt.screen.Synchronous = true
        'crt.session.Connect("/telnet " & ip)
        crt.session.Connect("/SSH2 /PASSWORD "& pass & username &ip) 
